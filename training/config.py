...
def train_epoch(model, optimizer, writer, train_dataloaders, use_loss, phase, epoch, multibranch):
    ...
    for batch, sample in enumerate(tqdm(train_dataloaders[phase])):
        ...
        labels = getLable(sample, use_loss, True)
        ...
        if multibranch:
            ...
            loss = sum([F.cross_entropy(out, labels) for out in outputs])
            _, preds = torch.max(sum([out.data for out in outputs]), 1)
...
def train_model(model, optimizer, scheduler, writer, train_dataloaders, num_epochs,
                use_model, use_loss, log_dir, lr_base, lr_class, multibranch):
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            train_epoch(model, optimizer, writer, train_dataloaders, use_loss, phase, epoch, multibranch)
